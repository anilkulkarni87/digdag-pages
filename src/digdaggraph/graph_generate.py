...
def _load_block_tree(root: Block, data: Optional[Dict[str, Any]], filepath: str, schedule_entries: List[ScheduleEntry]) -> None:
    root.penwidth = 1.0
    root.URL = ""
    ...
    for key in list(data.keys()):
        ...
        if key == "schedule":
            ...
            schedule_entries.append(ScheduleEntry(
                project=project, workflow=workflow_name, schedule_text=label, href=_workflow_html_href(filepath)
            ))
        ...
        if key == "td>":
            ...
            if sql_path:
                ...
                src_sql_abs = Path(os.getcwd()) / project / sql_path
                out_html_abs = Path(os.getcwd()) / GRAPHS_DIR / project / Path(sql_path).with_suffix(".html")
                ...
                href_from_workflow = os.path.relpath(out_html_abs, workflow_html_abs.parent).replace("\\", "/")
...
def generate_graph(input_filepath: str, output_dot_file: str) -> None:
    ...
    try:
        ...
        svg_text = "<svg xmlns='http://www.w3.org/2000/svg'><text x='10' y='20'>SVG read error</text></svg>"
